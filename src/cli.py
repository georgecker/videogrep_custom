import argparse
from videogrep import get_ngrams, sphinx, __version__
import videogrep_custom
...
def main():
    ...
    parser.add_argument(
        ...
        dest="search",
        action="append",
        help="search term - can be split with ';' exmpl: 'term one;term two; term three'",
    )
    parser.add_argument(
        "--search-type",
        "-st",
        dest="searchtype",
        default="sentence",
        choices=["sentence", "fragment", "mash", videogrep_custom.SEARCH_ORDERD],
        help="type of search - can either be 'sentence', 'fragment', 'mash' or 'orderd'",
    )
    parser.add_argument(
        "--max-clips",
        "-m",
    ...
    videogrep_custom.videogrep_custom(
